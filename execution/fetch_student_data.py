...
---------------------
Connects to SQLite database and fetches all records related to a given roll number and class.
...
def fetch_student_data(roll_no: str, class_name: str, student_name: str = None) -> dict:
    """
    Fetch all data for a student using a single SQLite connection for speed.
    """
    ...
    try:
        ...
        q1 = "SELECT roll_no, class_name, name FROM students WHERE roll_no = ? AND (UPPER(class_name) = ? OR UPPER(class_name) = ?)"
        cursor.execute(q1, (roll_no, cls_input, raw_cls.upper()))
        student_row = cursor.fetchone()

        # 2. Match roll_no and class starting with class_name
        if not student_row:
            q2 = "SELECT roll_no, class_name, name FROM students WHERE roll_no = ? AND UPPER(class_name) LIKE ?"
            cursor.execute(q2, (roll_no, f"{cls_input}%"))
            student_row = cursor.fetchone()

        # 3. Match by student_name if provided
        if not student_row and clean_student_name:
            q3 = "SELECT roll_no, class_name, name FROM students WHERE LOWER(name) LIKE ?"
            cursor.execute(q3, (f"%{clean_student_name.lower()}%",))
            student_row = cursor.fetchone()

        # 4. Match by roll_no alone
        if not student_row:
            q4 = "SELECT roll_no, class_name, name FROM students WHERE roll_no = ? LIMIT 1"
        ...
        if not student_row:
            ...
            try:
                cursor.execute(
                    "INSERT INTO students (roll_no, class_name, name) VALUES (?, ?, ?)",
        ...
        if clean_student_name and clean_student_name.lower() not in ["student", "unknown", "none", ""]:
            if final_name != clean_student_name:
                try:
                    cursor.execute(
                        "UPDATE students SET name = ? WHERE roll_no = ? AND UPPER(class_name) = ?",
        ...
        query_attendance = "SELECT date, status FROM attendance WHERE roll_no = ? AND (UPPER(class_name) = ? OR UPPER(class_name) = ?) ORDER BY date ASC"
        ...
        # Fetch Test Marks
        query_tests = "SELECT date, subject, marks FROM tests WHERE roll_no = ? AND (UPPER(class_name) = ? OR UPPER(class_name) = ?) ORDER BY date ASC"
        ...
        # Fetch Exam Marks
        query_exams = "SELECT date, subject, marks FROM exams WHERE roll_no = ? AND (UPPER(class_name) = ? OR UPPER(class_name) = ?) ORDER BY date ASC"
